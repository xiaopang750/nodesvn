...
define(function(require, exports, module){
	...
	lhgcalendar.fn = lhgcalendar.prototype =
	{
		...
		_setDate: function( day )
		{
			...
			that.day = day;
			nowDate = dateFormat.call( tmpDate, opt.format );
			replaceData = nowDate.replace(/\-/gi, "/");
			nowTime = new Date(replaceData).getTime();
			
			/* modify-by-fanwei 2014/9/15 */
			opt.onSetDate && opt.onSetDate.call( that ,nowDate, nowTime);
			that.inpE.value = nowDate;
	...
	/* modify-by-fanwei 2014/9/15 */
	function Calendar(config, event) {

		event = event || 'click';

		$(document).on(event, config.ele, function(){
			lhgcalendar( config );
			return false;
		});
